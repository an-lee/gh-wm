...
- **`wm-agent.yml` is generated**: Consumer repos get this file from `gh wm init` / `gh wm upgrade` (via `internal/gen/wmagent.go`). `gh wm upgrade` also runs **best-effort** `gh extension upgrade an-lee/gh-wm` before regenerating the workflow. **`gh wm update`** re-fetches `.wm/tasks/*.md` files that have a `source:` URL (set by `gh wm add <https-url>`). It calls into **this** repo's reusable workflows (`agent-resolve.yml`, `agent-run.yml`). Do not hand-edit generated caller files; change the template in `internal/gen/wmagent.go`.
